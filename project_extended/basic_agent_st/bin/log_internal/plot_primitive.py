...
# Plotta i dati
plt.figure(1)
plt.plot(dist, vel, color='blue', linewidth=4, label = 'Speed')

plt.title('Velocity primitives')
...
for i in range(0, len(c1), 10):
    ...
    plt.plot(valore_space[i, :], valore_speed[i, :], color='orange', linewidth=2)


#plt.axvline(x=Stop_distance, color='green', linestyle='--', linewidth=1.5)
ultima_posizione = len(dist) - 1

print("ultima posizione: ", ultima_posizione, "\n")
print("ultima dist: ", dist[ultima_posizione], "\n")

plt.axvline(x=dist[ultima_posizione], color='r', linestyle='--')

plt.legend(['Speed', 'Primitives'])

plt.savefig('primitive.svg', format='svg')
plt.show()
